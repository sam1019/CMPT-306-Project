...
public class Medic : Player {
		...
		foreach (GameObject p in GameManager.instance.playerList) {
			if(p.GetComponent<AlienShip>() != null){
				...
				if (temp.gridPosition == tile.gridPosition) { //Checks if tile selected contains enemy
					target = temp;
					MedicAttack.attackAlienShip(target);
				}
				else{
				...
				if (temp.gridPosition == tile.gridPosition) { //Checks if tile selected contains enemy
					target = temp;
					MedicAttack.attackAlienSoldier(target);
				}
				else{
					print("Target not found");
				}
				...
				if (temp.gridPosition == tile.gridPosition) { //Checks if tile selected contains enemy
					target = temp;
					MedicAttack.attackAlienSupport(target);
				}
				else{
					print("Target not found");
				}
				...
				if (temp.gridPosition == tile.gridPosition) { //Checks if tile selected contains enemy
					target = temp;
					MedicAttack.attackAlienBerserker(target);
				}
				else{
					print("Target not found");
				}
			...
			/**********TEST class************/
				...
				if (temp.gridPosition == tile.gridPosition) { //Checks if tile selected contains enemy
					target = temp;
					MedicAttack.attackAIPlayer(target);
				}
				else{
					print("Target not found");
				}
	...
	public virtual void TurnOnGUI(){
		...
		if (GUI.Button(buttonRect, "End Turn")) {
			//when end turn, disable move HightLigh
			//GameManager.instance.disableMoveHightLight();
			actionPoints = 2;
			moving = false;
			isAttacking = false;			
